...
public class MaterialCheckbox : MonoBehaviour,
    ...
    IPointerClickHandler
{
    ...
    [SerializeField] MaterialIcon m_iconGraphic;

    [Header("Style")]

    [SerializeField] Colors m_selectedColor = Colors.Primary;

    [SerializeField] Colors m_iconColor = Colors.OnPrimary;
    ...
    void Awake()
    {
        ...
        m_knobScale = new VarAnimator<float>(VarAnimator<float>.Lerp, 
            scale => {
                if (m_knobGraphic != null)
                    m_knobGraphic.transform.localScale = new Vector3(scale, 1f);
                ...
                transform.localScale = Vector3.one - Vector3.one * Mathf.Sin(scale * Mathf.PI) * 0.1f;
            }, m_animSpeed);
